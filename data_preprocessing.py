...
def get_image_data_generator(type="short"):
    if type == "short":
        image_data_generator = ImageDataGenerator(horizontal_flip=True)
        # brightness_range=[0.5, 1.5] - change mask too  # multiplier for hue of HSV colorspace
    ...
    if type == "middle":
        ...
        raise ValueError("Unexpected value for 'type'. Supported types : 'short', 'middle', 'full'")
...
def random_transform(image, rects, image_data_generator):
    ...
    for rect in rects:
        ...
        mask_transformed = image_data_generator.random_transform(mask, seed=seed)[..., 0]

        [i, j] = np.where(mask_transformed == 255)
